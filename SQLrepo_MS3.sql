...
BEGIN
	...
	SET @gravity = @gravity/1000;
	SET @thrust = @thrust/10000;
	...
	THEN
        ...
        THEN
            SET @velX = @velX + (@thrust * SIN(-RADIANS(@rotation)));
            SET @velY = @velY + (@thrust * COS(RADIANS(@rotation)));
...
BEGIN
    ...
    VALUES (playerName, landerName, mapName, TRUE, 0, 50, 0.1, 0, -90, @deltaFuel);
...
BEGIN
	...
	CALL generateMap('mars', 200, 235263, 7); -- generates map using fancy sine function and populates it with tiles
	CALL generateMap('venus', 400, 384572, 15); -- generates map using fancy sine function and populates it with tiles
